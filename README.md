...
- Pins each virtual gun's Raw Input device path, so lane N is `\\?\HID#HID_DEVICE_SYSTEM_VHF#2&56524c3N&0&0000#{378de44c-56ef-11d1-bc8c-00a0c91405dd}` on every PC and bindings that store device paths can be shared. Install writes each lane's `ParentIdPrefix` under `HKLM\SYSTEM\CurrentControlSet\Enum\VHF\HID_DEVICE_SYSTEM_VHF`, creating a lane once first if Windows has not seen it yet. If the paths are ever lost (a Windows feature upgrade), run install again. If install needs a reboot first, it says so; reboot and run install again.
